...
console.log('Writing project files to:', targetDir);

// 1. package.json
const packageJson = {
  "name": "enviro-civec-vanilla",
  "version": "1.0.0",
  "description": "Enviro Civec - Environmental & Civil Engineering Consulting Center",
  "type": "module",
  "scripts": {
    "build:ts": "tsc",
...
export class AutoCADCursor {
  ...
  private readonly interactiveSelector: string =
    'a, button, input, select, textarea, [role="button"], .interactive, [data-interactive="true"]';
  ...
  private createElements(): void {
    ...
    this.boxElement.style.cssText = 'position:absolute;width:12px;height:12px;border:2px solid var(--color-cursor, #ffffff);background:transparent;transition:border-color 0.1s, background-color 0.1s;pointer-events:none;';
    inner.appendChild(this.boxElement);

    const createLine = (style: string): HTMLDivElement => {
      const line = document.createElement('div');
      line.style.cssText = style + 'position:absolute;background-color:var(--color-cursor, #ffffff);pointer-events:none;';
      return line;
    };

    inner.appendChild(createLine('width:1.5px;height:40px;bottom:calc(50% + 6px);'));
    inner.appendChild(createLine('width:1.5px;height:40px;top:calc(50% + 6px);'));
    inner.appendChild(createLine('height:1.5px;width:40px;right:calc(50% + 6px);'));
    inner.appendChild(createLine('height:1.5px;width:40px;left:calc(50% + 6px);'));
  ...
  private onPointerMove = (e: PointerEvent): void => {
    ...
    if (hovering !== this.isHovering) {
      ...
      if (this.boxElement) {
        this.boxElement.style.borderColor = this.isHovering
          ? 'var(--accent, #b83324)'
          : 'var(--color-cursor, #ffffff)';
        this.boxElement.style.backgroundColor = this.isHovering
          ? 'rgba(184, 51, 36, 0.15)'
          : 'transparent';
      }
    }
...
export class AutoCADCursor {
  constructor(options = {}) {
    ...
    this.isTouch = false;
    this.rafId = 0;
    this.interactiveSelector = 'a, button, input, select, textarea, [role="button"], .interactive, [data-interactive="true"]';
    this.init();
  }
  ...
  onPointerMove(e) {
    ...
    if (hovering !== this.isHovering) {
      this.isHovering = hovering;
      if (this.boxElement) {
        this.boxElement.style.borderColor = this.isHovering ? 'var(--accent, #b83324)' : 'var(--color-cursor, #ffffff)';
        this.boxElement.style.backgroundColor = this.isHovering ? 'rgba(184, 51, 36, 0.15)' : 'transparent';
      }
    }
...
const translationsRaw = fs.readFileSync('d:\\Projects\\Website\\Enviro-Civec\\client\\src\\lib\\translations.ts', 'utf8');
const jsTranslationsContent = translationsRaw.replace('export const translations =', 'export const translations =');
fs.writeFileSync(path.join(targetDir, 'js', 'translations.js'), jsTranslationsContent, 'utf8');

console.log('Translations and Cursor written successfully.');
