...
export class AutoCADCursor {
  ...
  private updateColors(): void {
    const brownColor = 'var(--primary, #9b724c)';
    const defaultColor = 'var(--color-cursor, #ffffff)';

    if (this.boxElement) {
      this.boxElement.style.borderColor = this.isHovering ? brownColor : defaultColor;
      this.boxElement.style.backgroundColor = this.isHovering ? 'rgba(155, 114, 76, 0.22)' : 'transparent';
    }

    this.lines.forEach(line => {
      line.style.backgroundColor = this.isHovering ? brownColor : defaultColor;
    });
  }
...
console.log('AutoCAD Cursor updated with brand brown hover color across TS and JS!');
